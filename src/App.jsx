...
function AnimatedRoutes({ onOrder, onBooking, onSuccess }) {
  const location = useLocation();
  return (
    <AnimatePresence mode="wait">
      <motion.div key={location.pathname} variants={pageVariants} initial="initial" animate="animate" exit="exit">
        <Routes location={location}>
          <Route path="/" element={<Home openBooking={onBooking} />} />
          <Route path="/services" element={<Services openOrder={onOrder} openBooking={onBooking} />} />
          <Route path="/services/web-development" element={<WebDevelopment openOrder={onOrder} openBooking={onBooking} />} />
...
export function App() {
  ...
  return (
    <BrowserRouter>
      ...
      <main>
        <AnimatedRoutes
          onOrder={(plan, price) => setOrder({ plan, price })}
          onBooking={() => setBookingOpen(true)}
          onSuccess={setSuccess}
        />
      </main>
      <Footer openBooking={() => setBookingOpen(true)} />
      <a
      ...
      {order && (
        <OrderModal
          order={order}
          close={() => setOrder(null)}
          done={(message) => { setOrder(null); setSuccess(message); }}
        />
      )}
      {bookingOpen && (
        <BookingModal
          close={() => setBookingOpen(false)}
          done={(message) => { setBookingOpen(false); setSuccess(message); }}
        />
      )}
